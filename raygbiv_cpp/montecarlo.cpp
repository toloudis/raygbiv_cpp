...
inline float Pinv(float x) {
    return sqrt(4.0f * x);
}

vec3 uniformSampleHemisphere(float u, float v, float* pdf=nullptr) {
    auto z = u;
...
vec3 uniformSampleSphere(float u, float v, float* pdf = nullptr) {
    auto z = 1 - 2 * u;
    auto r = std::sqrt(std::max(0.0f, 1.0f - z * z));
    auto phi = 2 * pi * v;
    if (pdf) {
...
vec3 uniformSampleDisk(float u, float v, float* pdf = nullptr) {
    auto r = std::sqrt(u);
    auto theta = 2 * pi * v;
    if (pdf) {
        *pdf = 1.0 / pi;
...
vec3 cosineSampleHemisphere(float u, float v, float* pdf = nullptr) {
...
vec3 uniformSampleCone(float u, float v, float cosThetaMax, float* pdf=nullptr) {
    if (pdf) {
